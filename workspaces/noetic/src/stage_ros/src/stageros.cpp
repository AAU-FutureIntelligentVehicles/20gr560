...
//bypassing this shit... I hope that all hell doesnt break loose
nav_msgs::Odometry odom_msg;
Stg::Velocity v;
ros::Subscriber real_odom; //gets the postion of the robot from the robot "physical" odometry
...
void set_position(const nav_msgs::Odometry::ConstPtr& real_odom){
        odom_msg.pose.pose.position.x = real_odom->pose.pose.position.x;
        odom_msg.pose.pose.position.y = real_odom->pose.pose.position.x;
        odom_msg.pose.pose.orientation = real_odom->pose.pose.orientation;
}
...
StageNode::WorldCallback()
{
  if( ! ros::ok() ) {
    ...
    for (size_t r = 0; r < this->robotmodels_.size(); ++r)
    {
        ...
        // Translate into ROS message format and publish
        //some changes here as well in order to bypass
...
main(int argc, char** argv)
{
    ...
    //here we subscribe to the odometry of the golfcar
    
    ros::NodeHandle nh;
    real_odom = nh.subscribe("/odom_merge", 5, set_position);
